...
public class ProblemManager1 : MonoBehaviour
{
   ...
   public string mode; 
   public static ProblemManager1 Instance;
   ...
   public void LoadJHData()
   {
      problemsList.Clear();

      int num = 0;
      foreach (var q in JhAi.jhData.questions)
      {
         Problem p = new Problem();
         p.number = num++;
         p.type = EProblemType.S1;
         p.problem = q.question;
         p.options = new string[4];
         p.options[0] = q.options[0];
         p.options[1] = q.options[1];
         p.options[2] = q.options[2];
         p.options[3] = q.options[3];
         problemsList.Add(p);
      }
   }
